...
**Vim 9 is the primary editor** — MacVim for GUI work, terminal `vim` everywhere
else, across many machines. Neovim 0.12+ is supported and is where things are
slowly migrating, but Vim support is not negotiable: a change that only works in
Neovim must be additive and guarded, never a replacement.
